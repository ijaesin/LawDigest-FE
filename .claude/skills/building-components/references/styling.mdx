...
const Component = ({ className, ...props }: ComponentProps) => {
  ...
  return <div className={cn('rounded-lg border bg-white shadow-sm', isOpen && 'bg-blue-500', className)} {...props} />;
};
```
...
<button className={cn('px-4 py-2 rounded-lg', variant === 'primary' && 'bg-blue-500 text-white', className)} />;
```
...
function Button({ primary, className, ...props }) {
  return <button className={cn('px-4 py-2', primary ? 'bg-blue-500' : 'bg-gray-500', className)} {...props} />;
}
```
...
```tsx
const className = useMemo(() => cn(baseStyles, expensiveComputation(props), className), [props, className]);
```
