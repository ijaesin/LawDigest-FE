...
function Component<E extends React.ElementType = 'div'>({ as, children, ...props }: PolymorphicProps<E>) {
...
function Interactive({ as: Element = 'button', onClick, ...props }) {
  ...
  return <Element onClick={onClick} onKeyDown={Element !== 'button' ? handleKeyDown : undefined} {...props} />;
}
```
